...
if [ "$(whoami)" == "nodejs" ]; then
	...
	if sudo -v > /dev/null 2>&1; then
		...
		echo "Executing script to create ssl cert files..." &&
		sudo runuser -l nodejs -c '/opt/rfidmonitor/installation_resources/resources/prepare_ssl.sh' &&
		echo "Executing script to prepare pm2 to get the app online..." &&
		sudo runuser -l nodejs -c '/opt/rfidmonitor/installation_resources/resources/prepare_startup.sh' &&
		echo "Executing script to selftest" &&
		sudo runuser -l nodejs -c '/opt/rfidmonitor/installation_resources/resources/platform_selftest.sh' || exit 1;
	else
